...
export const MovieDetails = ({ cast = [], detalles }: props) => {
    ...
    return (
        ...
        <View style={{ marginHorizontal: 10 }}>
            ...
            <Text style={{color:"#fff"}}>{currency.format(detalles!.budget,{code:'USD'}) }</Text>
            <Text style={{color:"#fff"}}>{new Intl.NumberFormat('number', { style: 'currency', currency: 'EUR' }).format(detalles!.budget)}</Text>
        </View>
    )
}
